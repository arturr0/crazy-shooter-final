...
window.onload = function () {
  ...
  loop = function (currentTime) {
    ...
    for (let i = 0; i < obstacles.length; i++) if (obstacles != undefined) {
      ...
      var stone = new Image();
      stone.src = "stone.png";
      ctx1.drawImage(stone, obstacles[i].x, obstacles[i].y, obstacles[i].w, obstacles[i].h);
    ...
    for (var i = 0; i < gameAbbyses.length; i++) {
      if (player.x < gameAbbyses[i].x && player.x + 32 > gameAbbyses[i].x && movement_state == 'right') {
        abbysRight = true; abbysLeft = false;
        break;
      }

    }
    for (var i = 0; i < gameAbbyses.length; i++) {
      if (player.x < gameAbbyses[i].x + gameAbbyses[i].w && player.x + 32 > gameAbbyses[i].x + gameAbbyses[i].w && movement_state == 'left') {
        abbysLeft = true; abbysRight = false;
